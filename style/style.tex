...
\usepackage{datetime}
\usepackage{hyperxmp}% metadata (title, author, lang,...) for accessibility, etc.
\usepackage[pdfa,unicode=true,bookmarksdepth=subsection]{hyperref} % for automagic pdf links for toc, refs, etc.
\def\isolang{\IfLanguageName {finnish}{fi}{en}} %iso code (based on main language)
%TODO: author and keywords don't show in pdf properties?
%TODO: title shows in pdf porperties; but fail accessibility check?
\hypersetup{%
  pdfapart=1,
  pdfaconformance=b,
...
% Make citations to match Metropolia thesis guide
